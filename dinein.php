...
<body>
  ...
  <form action="thankyou.html" class="form" method="post">
        <h2>Your Details:</h2>
    ...
    function establishConnection()
    {
        ...
        function saveCustomerInfo()
        {
            ...
            $total = $_POST["total"];
            $sql = "INSERT INTO customer (Customer_ID,Customer_Name,Email_ID,Customer_Phone_no,Order_ID) VALUES($cust_id,'$fullName','$email',$mobileNumber,$order_id)";
            try {
                $rows = $conn->query($sql);
            } catch (PDOException $e) {
                echo "Query failed: " . $e->getMessage();
        ...
        function saveOrderInfo()
        {
            ...
            $type="Delivery";
            $status="Active";
            $sql = "INSERT INTO food_order (Order_ID, Order_Type, Order_Time, Order_Status, Total_Persons, Customer_ID) VALUES ($order_id, '$type', '$date', '$status', $total, $cust_id)";
            try {
                $rows = $conn->query($sql);
            } catch (PDOException $e) {
                echo "Query failed: " . $e->getMessage();
